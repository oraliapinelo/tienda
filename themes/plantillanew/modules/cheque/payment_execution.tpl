...
<h2><span>{l s='Cheque payment' mod='cheque'}</span></h2>
</div>
<form action="{$link->getModuleLink('cheque', 'validation', [], true)}" method="post">
		<div class="box-payment-style">
        <img src="{$img_dir}/cheque.jpg" alt="{l s='cheque' mod='cheque'}" width="86" height="54"  />
<em >	{l s='You have chosen to pay by cheque.' mod='cheque'}
		{l s='Here is a short summary of your order:' mod='cheque'}</em>
		<i class="icon-angle-right"></i> {l s='The total amount of your order is' mod='cheque'}
		...
		{else}
			{l s='We accept the following currency to be sent by cheque:' mod='cheque'}&nbsp;<b>{$currencies.0.name}</b><br />
			<input type="hidden" name="currency_payement" value="{$currencies.0.id_currency}" />
		{/if}
	<i class="icon-angle-right"></i> {l s='Cheque owner and address information will be displayed on the next page.' mod='cheque'}<br />
