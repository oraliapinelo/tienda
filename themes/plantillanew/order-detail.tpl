...
<div class="titled_box"> 
<h2><span>{l s='Follow your order step-by-step'}</span></h2>
</div>
...
<div id="order-detail-content" class="table_block">
	<table id="order-detail-content-table" class="std shop_table footable">
		...
		<tfoot>
			...
			<tr class="item">
				<td  colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					<strong>{l s='Total gift-wrapping:'}</strong> <span class="price-wrapping">{displayWtPriceWithCurrency price=$order->total_wrapping currency=$currency}</span>
				</td>
			</tr>
			{/if}
			<tr class="item">
			...
			{if !isset($product.deleted)}
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						<td class="td-1"><label for="cb_{$product.id_order_detail|intval}">{if $product.product_reference}{$product.product_reference|escape:'htmlall':'UTF-8'}{else}--{/if}</label></td>
						{if $return_allowed}<td class="order_cb"></td>{/if}
                        
						<td class="td-3"><input class="order_qte_input"  name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$product.customizationQuantityTotal|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$product.customizationQuantityTotal|intval}</span></label></td>
						{if $order->hasProductReturned()}
							<td>
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td class="td-3"><input class="order_qte_input" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" /><label for="cb_{$product.id_order_detail|intval}"><span class="order_qte_span editable">{$productQuantity|intval}</span></label></td>
						{if $order->hasProductReturned()}
							<td class="td-3">
								{$product['qty_returned']}
							</td>
...
{if $order->getShipping()|count > 0}
	<table id="shipping-table" class="std shop_table footable">
		...
		<tbody>
			...
			<tr class="item">
				<td class="tds-1">{$line.date_add}</td>
				<td class="tds-2">{$line.state_name}</td>
				<td class="tds-3">{if $line.weight > 0}{$line.weight|string_format:"%.3f"} {Configuration::get('PS_WEIGHT_UNIT')}{else}-{/if}</td>
				<td class="tds-4">{if $order->getTaxCalculationMethod() == $smarty.const.PS_TAX_INC}{displayPrice price=$line.shipping_cost_tax_incl currency=$currency->id}{else}{displayPrice price=$line.shipping_cost_tax_excl currency=$currency->id}{/if}</td>
				<td class="tds-5">
