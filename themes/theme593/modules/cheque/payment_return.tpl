...
{if $status == 'ok'}
	<p class="success">     {l s='Your order on %s is complete.' sprintf=$shop_name mod='cheque'}</p>
		...
		{l s='Please send us a cheque with:' mod='cheque'}<br />
   <ul>
	<li><i class="icon-angle-right"></i> {l s='an amount of' mod='cheque'} <span class="price"><strong>{$total_to_pay}</strong></span><br /></li>
	...
	<li><i class="icon-angle-right"></i> {l s='Do not forget to insert your order reference %s.' sprintf=$reference mod='cheque'}</li>
		{/if}
	<li><i class="icon-angle-right"></i> {l s='An e-mail has been sent to you with this information.' mod='cheque'}</li>
